...
- **A1b · Workflow de investigación guiado en el lead** *(v1 hecho)*. Panel **Investigación** en la ficha del lead: plantilla **fija según tipo** (campo = 10 ítems en 4 grupos: dueño+contacto/plano+mediciones/uso del suelo/acceso+servicios; urbano = 6), cada ítem cicla ⬜ falta → ✅ conseguido (con valor) → ⚠️ chequeado sin éxito, con cartel **"Próximo paso → …"** (primer pendiente; si el contacto falla, empuja a la alternativa) y contador `n/total`. Estado guardado como JSON en `CaptacionLead.investigacion_json`; plantilla en el front (`INV_TEMPLATES`). **Pendiente:** candidatos a propietario con confianza (patrón `PropietarioCatastral`), conectar con la parcela de catastro (item 12), y promover el dato verificado al Propietario del lead.
