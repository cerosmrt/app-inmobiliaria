...
- **A1b · Workflow de investigación guiado en el lead** *(EN CURSO)*. La etapa "Investigar" hoy no tiene nada propio. Se agrega un panel de **investigación por pasos lógicos**: una plantilla **fija según tipo** de datos a averiguar, cada ítem con estado ⬜ falta / ✅ conseguido (con valor) / ⚠️ chequeado sin éxito, y un **"próximo paso"** que resalta el primer pendiente. Para **campo**: dueño + contacto (con alternativa si falla), plano/mediciones, uso del suelo (parte sembrable), acceso y servicios. Estado por lead guardado como JSON en `CaptacionLead`; plantilla definida en el front. Reusa el patrón de confianza de `PropietarioCatastral` y se conectará luego con la parcela (item 12).
