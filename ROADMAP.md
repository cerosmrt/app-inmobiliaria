...
6d. **Exponer `Propiedad.notas` en la ficha.** El campo **ya existe en la base** (`models.py:169`, sale en `as_dict()`) pero **la ficha no lo muestra en ningún lado**: puede haber notas cargadas que nadie ve. Va en la columna privada, separado de la descripción (que es pública). *Por qué importa: es dato guardado e invisible — el peor estado posible.*
